...
# =====================================================================
# Ensure .env exists from container environment
# =====================================================================
ensure_env_from_container() {
    local env_file="$HERMES_HOME/.env"

    # If .env already exists and has API key, skip
    if [ -f "$env_file" ] && grep -q "MINIMAX_API_KEY=sk-" "$env_file" 2>/dev/null; then
        log_info "Using existing .env with API key"
        return 0
    fi

    log_step "Creating .env from container environment..."
...
"
    ...
    for var in $key_vars; do
        local value=""
        # Try container env first (explicit env vars override template strings)
        if [ -n "${!var:-}" ]; then
            value="${!var}"
        ...
        if [ -n "$value" ]; then
            ...
            log_info "  $var: from container env"
        fi
    done

    # Ensure MINIMAX_API_KEY is set (prefer ANTHROPIC_AUTH_TOKEN if MINIMAX is template)
    if ! grep -q "MINIMAX_API_KEY=sk-" "$env_file" 2>/dev/null; then
        if grep -q "ANTHROPIC_AUTH_TOKEN=sk-" "$env_file" 2>/dev/null; then
            local anthropic_key=$(grep "ANTHROPIC_AUTH_TOKEN=" "$env_file" | cut -d'=' -f2)
            echo "MINIMAX_API_KEY=${anthropic_key}" >> "$env_file"
            log_info "  MINIMAX_API_KEY: derived from ANTHROPIC_AUTH_TOKEN"
        fi
    fi
...
if [ -f "$PRESETS_DIR/config.yaml" ]; then
    ...
    tmp_cfg="/tmp/hermes-config-$$.yaml"
    load_env
