...
// Random OS choices for fingerprint spoofing - rotate on each browser restart
// Note: 'ios' and 'linux' not supported by Camoufox - removed
const SPOOF_OS_OPTIONS = ['windows', 'macos', 'android'];
let currentSpoofOS = SPOOF_OS_OPTIONS[Math.floor(Math.random() * SPOOF_OS_OPTIONS.length)];

// GPU spoofing options - MUST match the spoof OS for compatibility
const OS_GPU_PAIRS = {
  // Windows GPUs
  'windows': [
...
// Load GPU config from launch-config.json if it exists
function loadLaunchConfig() {
  try {
    const fs = require('fs');
    const configPath = '/root/stealth-browser/camoufox-source/launch-config.json';
...
// Select random GPU for the given OS
function selectGPUForOS(os) {
  const gpus = OS_GPU_PAIRS[os] || OS_GPU_PAIRS['windows'];
...
async function launchBrowserInstance() {
  ...
  // Rotate to new random OS for each browser launch (anti-fingerprinting)
  currentSpoofOS = SPOOF_OS_OPTIONS[Math.floor(Math.random() * SPOOF_OS_OPTIONS.length)];
  
  // Select GPU - use config if available, otherwise select GPU that matches the spoof OS
  // webgl_config format is [vendor, renderer] (OS is passed separately via os: param)
  let gpuConfig = null;
  let webgl_config = null;
  
  if (launchConfig?.gpu_config && Array.isArray(launchConfig.gpu_config) && launchConfig.gpu_config.length >= 2) {
    gpuConfig = { vendor: launchConfig.gpu_config[0], renderer: launchConfig.gpu_config[1] };
    webgl_config = [gpuConfig.vendor, gpuConfig.renderer];
  } else {
    // Fall back to GPU that matches the spoof OS
    gpuConfig = selectGPUForOS(currentSpoofOS);
    webgl_config = [gpuConfig.vendor, gpuConfig.renderer];
  }
  
  log('info', 'launching camoufox', { 
    hostOS, 
    spoofOS: currentSpoofOS, 
    geoip: !!proxy,
    gpu: gpuConfig ? `${gpuConfig.vendor} / ${gpuConfig.renderer}` : 'auto-random'
  });
  
  // Firefox prefs to further harden fingerprint
  // (stored for potential future use - currently Camoufox handles most of this)
  
  const options = await launchOptions({
    headless: true,
    os: currentSpoofOS,  // Use SPOOFED OS, not real host OS
    humanize: true,
    enable_cache: true,
    proxy: proxy,
    geoip: !!proxy,
    webgl_config: webgl_config,  // Spoof GPU fingerprint
    // Note: firefox_user_prefs would need Camoufox to pass them through
  });
