# aatosteam CLI Complete Reference

## Global Options

```
aatosteam [--version] [--json] [--data-dir PATH] <command>
```

- `--json` — Output JSON instead of human-readable text. Apply before subcommand: `aatosteam --json team discover`
- `--data-dir PATH` — Override data directory (default: `~/.aatosteam`)

## Environment Variables

aatosteam agents use these environment variables for identity:

| Variable | Description | Example |
|----------|-------------|---------|
| `aatosteam_AGENT_ID` | Unique agent identifier | `a1b2c3d4e5f6` |
| `aatosteam_AGENT_NAME` | Human-readable agent name | `alice` |
| `aatosteam_AGENT_TYPE` | Agent role type | `leader`, `general-purpose`, `researcher` |
| `aatosteam_TEAM_NAME` | Team the agent belongs to | `dev-team` |
| `aatosteam_DATA_DIR` | Override data directory | `/tmp/aatosteam-data` |

When spawning agents via `aatosteam spawn`, these are set automatically.
Legacy `OH_*` aliases remain accepted for compatibility.

---

## Team Commands (`aatosteam team`)
...
```bash
aatosteam team spawn-team <name> [options]
```
...
Example:
```bash
aatosteam team spawn-team dev-team -d "Backend development team" -n alice
```
...
```bash
aatosteam team request-join <team> <proposed-name> [options]
```
...
```bash
aatosteam team approve-join <team> <request-id> [--assigned-name NAME]
```
...
```bash
aatosteam team reject-join <team> <request-id> [--reason TEXT]
```
...
```bash
aatosteam team cleanup <team> [--force]
```

---

## Inbox Commands (`aatosteam inbox`)
...
```bash
aatosteam inbox send <team> <to> <content> [options]
```
...
```bash
aatosteam inbox broadcast <team> <content> [options]
```
...
```bash
aatosteam inbox receive <team> [options]
```
...
```bash
aatosteam inbox peek <team> [--agent NAME]
```
...
```bash
aatosteam inbox watch <team> [--agent NAME] [--poll-interval 1.0]
```

---

## Task Commands (`aatosteam task`)
...
```bash
aatosteam task create <team> <subject> [options]
```
...
Example:
```bash
aatosteam task create dev-team "Implement auth" -o alice -d "Add JWT authentication"
```
...
```bash
aatosteam task get <team> <task-id>
```
...
```bash
aatosteam task update <team> <task-id> [options]
```
...
```bash
aatosteam task list <team> [--status STATUS] [--owner NAME] [--priority LEVEL] [--sort-priority]
```

---

## Board Commands (`aatosteam board`)
...
aatosteam --json board show <team>
```
...
```bash
aatosteam board live <team> [--interval 2.0]
```

---

## Plan Commands (`aatosteam plan`)
...
```bash
aatosteam plan submit <team> <agent> <plan-content-or-file> [--summary TEXT]
```
...
```bash
aatosteam plan approve <team> <plan-id> <agent> [--feedback TEXT]
```
...
```bash
aatosteam plan reject <team> <plan-id> <agent> [--feedback TEXT]
```

---

## Lifecycle Commands (`aatosteam lifecycle`)
...
```bash
aatosteam lifecycle request-shutdown <team> <from-agent> <to-agent> [--reason TEXT]
```
...
```bash
aatosteam lifecycle approve-shutdown <team> <request-id> <agent>
```
...
```bash
aatosteam lifecycle reject-shutdown <team> <request-id> <agent> [--reason TEXT]
```
...
```bash
aatosteam lifecycle idle <team> [--last-task ID] [--task-status STATUS]
```
...
```bash
aatosteam spawn <backend> <command...> [options]
```
...
Example:
```bash
aatosteam spawn subprocess claude --team dev-team --agent-name bob --agent-type researcher
```

---

## Identity Commands (`aatosteam identity`)
...
```bash
eval $(aatosteam identity set --agent-name alice --team dev-team)
```
